...
For normal deployments you should not need to edit this file — `.env` overrides handle identity and credentials. If you do need to change operational behavior (LLM models, limits, timeouts), edit `config/agent.yaml` directly. After pulling updates, review incoming changes to this file and resolve any conflicts with your local modifications.
