...
TOOL_GUARDRAILS: dict[str, dict[str, Any]] = {
    ...
    "Notes": {
        ...
        "text": (
            "Agent notes are never injected into prompts. Use for your own reference, research drafts, "
            "or private working context. Use note_search and note_read later to retrieve them."
        ),
    },
    "Contacts": {
...
class ToolRuntime:
    ...
    def context_search(self, query: str, limit: int = 10) -> dict[str, Any]:
        clean_query = str(query or "").strip()
        if not clean_query:
            raise ToolError("query is required")
    ...
    def memory_remember(self, content: str, tags: Optional[list[str]] = None, metadata: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    def memory_search(self, query: str = "", limit: int = 10) -> dict[str, Any]:
        rows = MemoryStore(self.db, self.config).keyword_search(query=query, limit=self._bounded_limit(limit))
    ...
    def note_create(
        ...
        row = NoteStore(self.db, self.config).create(
            title=title,
            content=content,
            tags=tags,
            metadata=metadata,
        ...
        return {"note": {key: row.get(key) for key in ("id", "title", "tags", "metadata", "created_at", "updated_at")}}

    def note_search(self, query: str = "", tags: Optional[list[str]] = None, limit: int = 10) -> dict[str, Any]:
    ...
    def note_update(
        ...
        fields_present = [value is not None for value in (content, title, tags, metadata)]
        if not any(fields_present):
            raise ToolError("at least one note field must be provided")
        row = NoteStore(self.db, self.config).update(
            note_id=note_id,
            content=content,
            title=title,
            tags=tags,
        ...
        return {"note": {key: row.get(key) for key in ("id", "title", "tags", "metadata", "updated_at")}}
...
FUNCTION_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "note_create",
            "description": "Create an agent note. Notes are never injected into prompts; use note_search and note_read later to retrieve them.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {"type": "string"},
    ...
    },
    {
        ...
        "function": {
            "name": "note_search",
            "description": "Search agent notes semantically and by keyword. Returns note IDs and snippets only; call note_read for full content.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
