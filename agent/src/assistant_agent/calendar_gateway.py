...
class CalendarGateway:
    ...
    def _parse_ics_file(
        ...
        for lines in self._vevent_blocks(raw):
            ...
            if managed_row and event.has_managed_marker() and event.marker_id() == str(managed_row.get("assistant_id")):
    ...
    def _render_event(
        ...
        lines = [
            ...
            "%s:%s" % ("DTSTART;VALUE=DATE" if all_day else "DTSTART", self._format_event_time(start, all_day)),
            "%s:%s" % ("DTEND;VALUE=DATE" if all_day else "DTEND", self._format_event_time(end, all_day)),
        ...
        for attendee in attendees:
            lines.append("ATTENDEE:mailto:%s" % attendee)
    ...
    def _managed_event_from_row(self, row: dict[str, Any]) -> CalendarEvent:
        ...
        event = events[0]
        if not event.has_managed_marker() or event.marker_id() != str(row.get("assistant_id")):
            raise CalendarError("managed calendar event marker is missing or does not match")
        return event
