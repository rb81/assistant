...
class ContextStore:
    ...
    def search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if query_embedding is not None:
            return self._semantic_search(clean_query, query_embedding, limit)
        return self._keyword_search(clean_query, limit)
    ...
    def _semantic_search(self, query: str, query_embedding: list[float], limit: int) -> list[dict[str, Any]]:
        ...
        # Search each source
        all_results.extend(self._search_memories_semantic(query_embedding, per_source))
        all_results.extend(self._search_jobs_semantic(query_embedding, per_source, days))
        all_results.extend(self._search_reminders_semantic(query_embedding, per_source, days))
        all_results.extend(self._search_outbound_emails_semantic(query_embedding, per_source, days))
        all_results.extend(self._search_inbound_emails_semantic(query_embedding, per_source, days))
        all_results.extend(self._search_notes_semantic(query_embedding, per_source))
        all_results.extend(self._search_projects_semantic(query_embedding, per_source, days))
        all_results.extend(self._search_contacts_semantic(query_embedding, per_source))
    ...
    def _search_memories_semantic(self, query_embedding: list[float], limit: int) -> list[dict[str, Any]]:
        ...
        return self._score_and_format(rows, query_embedding, "memory", limit, self._format_memory)

    def _search_jobs_semantic(self, query_embedding: list[float], limit: int, days: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "job", limit, self._format_job)

    def _search_reminders_semantic(self, query_embedding: list[float], limit: int, days: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "reminder", limit, self._format_reminder)

    def _search_outbound_emails_semantic(self, query_embedding: list[float], limit: int, days: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "outbound_email", limit, self._format_outbound_email)

    def _search_inbound_emails_semantic(self, query_embedding: list[float], limit: int, days: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "inbound_email", limit, self._format_inbound_email)

    def _search_notes_semantic(self, query_embedding: list[float], limit: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "note", limit, self._format_note)

    def _search_projects_semantic(self, query_embedding: list[float], limit: int, days: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "project", limit, self._format_project)

    def _search_contacts_semantic(self, query_embedding: list[float], limit: int) -> list[dict[str, Any]]:
        rows = self.db.fetch_all(
            """
        ...
        return self._score_and_format(rows, query_embedding, "contact", limit, self._format_contact)
